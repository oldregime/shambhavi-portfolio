...
export const ExperiencePage: React.FC<PageProps> = ({ onNavigate }) => {
  return (
    <div className="h-full flex flex-col justify-between py-2 sm:py-3 space-y-3 sm:space-y-4 overflow-hidden">
      ...
      <div className="space-y-3 flex-1 my-auto overflow-hidden">
        {EXPERIENCES.map((exp) => (
          <div
            key={exp.company}
            className="p-3.5 sm:p-4 rounded-2xl bg-[#fffef9] border-2 border-[#3f2a1e] shadow-xs space-y-2 relative"
            style={{ filter: 'url(#sketch-wobble)' }}
          >
            <div className="flex items-center justify-between border-b border-[#ede0ca] pb-1.5">
              <div>
            ...
            <p className="text-xs text-[#2b1d19] leading-relaxed font-sans">
              {exp.description}
            </p>
